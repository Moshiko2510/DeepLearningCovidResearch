...
for i in range(1):
    ...
    model.add(layers.Conv2D(8, (3, 3), strides=1, padding='same', input_shape=(256, 256, 3)))
    ...
    model.add(layers.Conv2D(16, (1, 1), strides=1, padding='same'))
    model.add(layers.BatchNormalization())
    model.add(LeakyReLU(alpha=0.1))
    ...
    model.add(layers.Conv2D(32, (1, 1), strides=1, padding='same'))
    model.add(layers.BatchNormalization())
    model.add(LeakyReLU(alpha=0.1))
    ...
    model.compile(optimizer=optimizers.Adam(lr=LEARNING_RATE), loss='categorical_crossentropy', metrics=['acc'])
    if (i==0):
        model.summary()
